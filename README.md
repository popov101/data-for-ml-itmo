...
P.S. Разметка 1000 изображений заняла 40-60 минут.
...
# Домашнее задание №4: Мультимодальные данные

## Выполненные задачи

### 1. Загрузка и предварительная обработка изображений
- Скачано 4694/4702 главных фото объявлений (8 URL недоступны)
- Удалены записи с отсутствующими изображениями
- Создана директория для хранения изображений

### 2. Анализ мультимодальных признаков
#### Текстовые данные:
- Средняя длина признаков:
  - TITLE: 24.86 символов
  - DESCRIPTION: 283.91 символов 
  - ADDRESS: 41.43 символов
- Построено облако слов для описаний

#### Изображения:
- Визуализированы примеры фотографий с ценами и площадями
- Выявлена корреляция между качеством ремонта и ценой

### 3. Нормализация данных
#### Изображения:
- Приведены к размеру 224x224 пикселей
- Нормализованы (значения [0, 1])
- Сохранены в .npy формате

#### Текстовые эмбеддинги:
- Сгенерированы с помощью BERT (bert-base-multilingual-cased)
- Размерность: 4694x768
- Сохранены в `description_embeddings.npy`

### 4. Предложена структура и принцип связки данных в датасете

### 5. Предложены варианты использования и концепция модели
