...
def collect_offers(max_pages=100, min_delay=2, max_delay=5):
    ...
    # Проверяем доступность API и получаем общее количество объявлений
    try:
    ...
    # Для отслеживания уже обработанных страниц
    processed_pages = set()
    ...
    with tqdm(total=max_pages, desc="Сбор данных") as pbar:
        for page in pages:
            ...
            try:
                ...
                # Логирование
                print(f"\n[+] Страница {page}: собрано {len(page_data)} объявлений")
